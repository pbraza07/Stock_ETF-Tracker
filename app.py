...
def load_snapshot() -> pd.DataFrame:
    ...
    v5.2 does not bundle data/market_snapshot.csv, so upgrades can no longer
...
with st.sidebar:
    ...
    st.caption("Fast startup uses the last durable snapshot. v5.2 never overwrites generated snapshot data during upgrades; Refresh seeds from GitHub first, then requests fresh history. The full scheduled refresh runs every day at 6:00 PM America/New_York.")
...
if refresh_now:
    ...
    # This is the v5.2 safeguard that keeps returns visible even if Render's IP
