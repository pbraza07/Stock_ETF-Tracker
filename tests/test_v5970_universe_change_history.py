...
def test_release_version_5970():
    assert (ROOT / "VERSION.txt").read_text(encoding="utf-8").strip() == "5.11.11"
    assert "v5.11.11" in APP


def test_append_only_history_uses_upgrade_safe_bootstrap():
    # Release overlays must never replace the user's append-only live ledger.
    path = ROOT / "data" / "universe_change_history.bootstrap.json"
    assert path.exists()
    assert isinstance(json.loads(path.read_text(encoding="utf-8")), list)
    assert not (ROOT / "data" / "universe_change_history.json").exists()
