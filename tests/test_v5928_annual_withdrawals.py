...
def test_release_version_is_5928():
    assert (ROOT / "VERSION.txt").read_text().strip() == "5.9.47"
...
def test_withdrawal_ui_and_persistence_contract_present():
    ...
    assert 'MarketScope Portfolio Split Simulator v10 - actual monthly returns + monthly/yearly rebalanced/not-rebalanced withdrawal results' in APP
