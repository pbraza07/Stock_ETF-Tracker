...
def _portfolio_annual_withdrawal_schedule(
    ...
    """Run a portfolio-level annual withdrawal path using the selected instruments' actual saved returns.

    Completed calendar years are processed from oldest to newest. Each instrument first receives its
    own annual return, then the requested withdrawal is taken proportionally from all remaining
    instrument balances. This preserves the portfolio's post-return relative weights without assuming
    an artificial annual rebalance. Current YTD, when requested, is shown as a final partial-period
    row and does not trigger an additional annual withdrawal.
    """
    try:
    ...
    selected_years = list(YEAR_RETURN_COLS[:years_requested])
    if len(selected_years) < years_requested:
        return {"unavailable": True, "reason": "The requested completed-year horizon is not available."}
...
with portfolio_tab:
    ...
    with portfolio_build_tab:
        ...
        # v5.9.31 - only offer historical horizons that every selected instrument can actually support.
        # This prevents pre-IPO / pre-inception years from becoming an impossible portfolio request.
        common_history_years = 20
        history_limiters: list[tuple[str, int]] = []
        if selected_portfolio_symbols:
            _portfolio_history_lookup = market.copy()
            _portfolio_history_lookup["Symbol"] = _portfolio_history_lookup["Symbol"].astype(str).str.upper()
            _portfolio_history_lookup = _portfolio_history_lookup.drop_duplicates("Symbol", keep="first").set_index("Symbol", drop=False)
            common_history_years = 20
            for _sym in selected_portfolio_symbols:
                _available = 0
                if _sym in _portfolio_history_lookup.index:
                    _hist_row = _portfolio_history_lookup.loc[_sym]
                    if isinstance(_hist_row, pd.DataFrame):
                        _hist_row = _hist_row.iloc[0]
                    for _year in YEAR_RETURN_COLS:
                        _raw = pd.to_numeric(pd.Series([_hist_row.get(_year)]), errors="coerce").iloc[0]
                        if pd.isna(_raw) or not np.isfinite(_raw):
                            break
                        _available += 1
                history_limiters.append((_sym, _available))
                common_history_years = min(common_history_years, _available)
        else:
            common_history_years = 20

        portfolio_period_options = ["YTD", *[f"{i}Y" for i in range(1, common_history_years + 1)]]
        _saved_period = str(st.session_state.get("portfolio_period") or "YTD")
        if _saved_period not in portfolio_period_options:
            st.session_state["portfolio_period"] = f"{common_history_years}Y" if common_history_years > 0 else "YTD"
        ...
        with port_controls[0]:
            ...
            if selected_portfolio_symbols and common_history_years < 20:
                _limiting = [sym for sym, yrs in history_limiters if yrs == common_history_years]
                if common_history_years > 0:
                    st.caption(
                        f"Common completed-year history: {common_history_years}Y. Limited by {', '.join(_limiting[:4])}"
                        + ("…" if len(_limiting) > 4 else "")
                        + ". Older pre-IPO/pre-inception years are automatically excluded."
                    )
                else:
                    st.caption(
                        "No completed calendar-year history is shared by every selected instrument yet. Use YTD or remove the newest instrument."
                    )
        ...
        if selected_portfolio_symbols:
            ...
            if not allocation_valid:
                ...
                # v5.9.28 - optional annual-withdrawal path. The withdrawal is applied after each
                # completed calendar year's actual saved return, with cash removed proportionally
                # from the post-return instrument balances.
                if portfolio_withdrawals_enabled and portfolio_annual_withdrawal > 0 and unresolved_amount <= 0.005:
                    portfolio_withdrawal_result = _portfolio_annual_withdrawal_schedule(
                        market,
                    ...
                    if portfolio_withdrawal_result.get("unavailable"):
                        st.warning(f"Annual withdrawal schedule unavailable: {portfolio_withdrawal_result.get('reason', 'required annual return data is unavailable')} ")
                    else:
                        st.markdown("<div class='portfolio-analytics-title'>ANNUAL WITHDRAWAL BALANCE SCHEDULE</div>", unsafe_allow_html=True)
                        st.caption(
                            "Each row applies that year's saved instrument returns first, then subtracts the requested withdrawal. "
                            "The balance shown is what remains invested after the withdrawal. Current YTD, when enabled, is a partial-period row and does not trigger another annual withdrawal."
                        )
                        wc1, wc2, wc3, wc4 = st.columns(4)
                        wc1.metric("Annual withdrawal", f"${float(portfolio_annual_withdrawal):,.2f}")
                        wc2.metric("Total withdrawn", f"${float(portfolio_withdrawal_result.get('total_withdrawn') or 0):,.2f}")
                        wc3.metric("Portfolio remaining", f"${float(portfolio_withdrawal_result.get('ending_balance') or 0):,.2f}")
                        wc4.metric(
                            "Remaining + withdrawals",
                            f"${float(portfolio_withdrawal_result.get('net_value_including_withdrawals') or 0):,.2f}",
                            delta=f"${float(portfolio_withdrawal_result.get('net_profit_including_withdrawals') or 0):+,.2f}",
                        )
                        withdrawal_rows = []
                        for row in portfolio_withdrawal_result.get("schedule") or []:
                        ...
                        if withdrawal_rows:
                            st.dataframe(
                                ...
                                use_container_width=True,
                                hide_index=True,
                                height=min(560, 56 + 36 * len(withdrawal_rows)),
                            )
                        if portfolio_withdrawal_result.get("depleted_year"):
                            st.error(
                                f"The portfolio is depleted during {portfolio_withdrawal_result.get('depleted_year')} under this withdrawal amount. "
                                "Later annual withdrawals cannot be funded."
                            )
    ...
    with portfolio_manage_tab:
        ...
        if save_simulation_clicked and portfolio_save_ready:
            ...
            record = {
                ...
                "withdrawal_total": float(portfolio_withdrawal_result.get("total_withdrawn") or 0) if portfolio_withdrawals_enabled else 0.0,
                "withdrawal_ending_balance": float(portfolio_withdrawal_result.get("ending_balance") or 0) if portfolio_withdrawals_enabled else None,
                "withdrawal_net_value": float(portfolio_withdrawal_result.get("net_value_including_withdrawals") or 0) if portfolio_withdrawals_enabled else None,
                "withdrawal_net_profit": float(portfolio_withdrawal_result.get("net_profit_including_withdrawals") or 0) if portfolio_withdrawals_enabled else None,
                "withdrawal_depleted_year": portfolio_withdrawal_result.get("depleted_year") if portfolio_withdrawals_enabled else None,
                "withdrawal_schedule": list(portfolio_withdrawal_result.get("schedule") or []) if portfolio_withdrawals_enabled else [],
                "pdf_layout": "MarketScope Portfolio Split Simulator v6 - annual withdrawal schedule + required instrument market data on page 1",
            }
...
with sector_tab:
    ...
    if sector_stocks.empty:
            ...
            for col, (_, sector_row) in zip(sector_cols, sector_df.iloc[start:start+3].iterrows()):
                with col:
                    ...
                    with st.container(border=True, key=f"sector_card_{sector_button_key}"):
                        ...
                        with kpi_stocks:
                            if st.button(
                                f'📈 {int(sector_row["Stocks"])} stocks\nView top performers',
                                key=f'sector_stock_count_{sector_button_key}',
                                use_container_width=True,
                                type='primary' if st.session_state.sector_drilldown == str(sector_row["Sector"]) else 'secondary',
                                help=f'Open the top-performing stocks in {sector_row["Sector"]}.',
                            ):
                                selected_sector = str(sector_row["Sector"])
                                st.session_state.sector_drilldown = None if st.session_state.sector_drilldown == selected_sector else selected_sector
                                st.rerun()
                        with kpi_cap:
                            st.markdown(
                        ...
                        st.markdown(
                            ...
                            + ''.join(
                                '<div class="perf-cell"><span>' + escape(metric) + '</span><b class="' + _tone(sector_row.get(metric)) + '">' + escape(format_pct(sector_row.get(metric))) + '</b></div>'
                                for metric in PERF_COLS
                            )
                            + '</div>',
                            unsafe_allow_html=True,
        ...
        if st.session_state.sector_drilldown:
            drill_sector = str(st.session_state.sector_drilldown)
            drill = sector_stocks.loc[sector_stocks["Sector"].astype(str).eq(drill_sector)].copy()
            if not drill.empty:
                drill["_rank"] = pd.to_numeric(drill.get(sector_sort_period), errors="coerce")
                drill = drill.sort_values(["_rank", "MarketCap"], ascending=[False, False], na_position="last").head(10)
                ...
                with st.container(border=True, key="sector_top_performers_box"):
                    dh1, dh2 = st.columns([4, 1])
                    with dh1:
                        st.markdown(f"#### {drill_sector} • Top performers")
                        st.caption(f"Top {len(drill)} stocks ranked by {sector_sort_period}. Click another sector stock-count button to switch sectors.")
                    with dh2:
                        if st.button("✕ Close", key="close_sector_drilldown", use_container_width=True):
                            st.session_state.sector_drilldown = None
                            st.rerun()
                    drill_table = drill.copy()
                    drill_table["Logo"] = drill_table["Symbol"].astype(str).str.upper().map(lambda sym: drill_logo_urls.get(sym, ""))
                    avg_target = pd.to_numeric(drill_table.get("Price Target Average"), errors="coerce")
                    cur_price = pd.to_numeric(drill_table.get("Price"), errors="coerce")
                    drill_table["Avg Target Implied %"] = np.where(
                        avg_target.notna() & cur_price.notna() & (cur_price > 0),
                        (avg_target / cur_price - 1.0) * 100.0,
                        np.nan,
                    )
                    drill_cols = list(dict.fromkeys([
                        "Logo", "Symbol", "Name", "Analyst Rating", "Price", sector_sort_period,
                        "YTD", "1Y", "Price Target Average", "Avg Target Implied %", "MarketCap",
                    ]))
                    ...
                    st.dataframe(
                        ...
                        use_container_width=True,
                        hide_index=True,
                        height=min(460, 80 + len(drill_table) * 36),
                        column_config={
                            "Logo": st.column_config.ImageColumn("Logo", width="small"),
                            "Price": st.column_config.NumberColumn("Current Price", format="$%.2f"),
                            sector_sort_period: st.column_config.NumberColumn(sector_sort_period, format="%.2f%%"),
                            ...
                            "Avg Target Implied %": st.column_config.NumberColumn("Target Upside", format="%.2f%%"),
                            "MarketCap": st.column_config.NumberColumn("Market Cap", format="$%.0f"),
                        },
                        key="sector_top_performers_table",
                    )
        ...
        st.dataframe(
            ...
            column_config={
                ...
                **{metric: st.column_config.NumberColumn(metric, format="%.2f%%") for metric in PERF_COLS},
            },
        )
