...
sim_cols = st.columns([1.2, 2.6, 1.2, 2.4])
with sim_cols[0]:
...
with sim_cols[1]:
    investment_year_choice = st.segmented_control(
        "Investment years",
        [f"{i}Y" for i in range(1, 11)],
        default="10Y",
        key="investment_year_choice",
        help="Select how many of the most recent completed calendar years to compound.",
    )
    investment_years = int(str(investment_year_choice or "10Y").replace("Y", ""))
with sim_cols[2]:
    include_current_ytd = st.toggle(
        "Include current YTD",
        value=True,
        key="include_current_ytd",
        help="When enabled, the current YTD return is applied after the selected completed calendar years to estimate value today.",
    )
with sim_cols[3]:
    st.caption(
        "Choose 1–10 completed calendar years. The simulator compounds the actual year-by-year adjusted returns in chronological order. "
...
def _instrument_card_html(row: pd.Series, price: str, cap_display: str, rating: str, signal: str) -> str:
    ...
    parts = [
        '<div class="instrument-card full-metrics-card">',
...
def _detail_performance_html(row: pd.Series) -> str:
    # Explicit source order is preserved by CSS grid on desktop and mobile.
    return '<div class="detail-performance-grid">' + _performance_cells(row) + '</div>'
...
- **Investment simulator:** choose an investment amount and **1–10 completed years**. MarketScope compounds exactly that many most-recent contiguous calendar-year returns for each instrument; if the selected history is not available, the result is shown as unavailable instead of shortening the horizon silently. By default the current YTD return is then applied to estimate a current value. No deposits, withdrawals, taxes, fees, or future returns are assumed.
