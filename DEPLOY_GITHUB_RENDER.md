...
## IMPORTANT — upgrade from v5/v5.1

Do **not** delete the existing generated files in GitHub before uploading v5.2:

- `data/default_universe.csv`
- `data/market_snapshot.csv`
- `data/snapshot_metadata.json`

The v5.2 ZIP intentionally does not contain those paths, so a normal GitHub **Add files → Upload files** upgrade leaves the populated data untouched. Upload all v5.2 files on top of the repository and commit. GitHub Actions will continue refreshing the generated files at 6:00 PM U.S. Eastern every day.

On a brand-new repository, the bundled `*.bootstrap.*` files let the app start while the first GitHub Action creates the full generated universe and snapshot.

# Deploy MarketScope v5.2 to GitHub + Render
