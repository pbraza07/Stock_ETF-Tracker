...
if st.button(
    "◫ Portfolio Split Simulator" if not st.session_state.portfolio_simulator_open else "◫ Hide Portfolio Split Simulator",
    key="toggle_portfolio_simulator",
    use_container_width=True,
    type="primary" if st.session_state.portfolio_simulator_open else "secondary",
):
    st.session_state.portfolio_simulator_open = not st.session_state.portfolio_simulator_open
    st.rerun()

if st.session_state.portfolio_simulator_open:
...
# Save / Manage portfolio simulations — independently collapsible.
_saved_count_preview = len(cached_saved_simulations())
if st.button(
    f"💾 Save / Manage Portfolio Simulations ({_saved_count_preview} saved)" if not st.session_state.portfolio_manager_open else "💾 Hide Save / Manage Portfolio Simulations",
    key="toggle_portfolio_manager",
    use_container_width=True,
    type="primary" if st.session_state.portfolio_manager_open else "secondary",
):
    st.session_state.portfolio_manager_open = not st.session_state.portfolio_manager_open
    if st.session_state.portfolio_manager_open:
...
with card_view_tab:
    ...
    def render_card_profit_period_fragment(row_data: dict, principal: float) -> None:
        """Update a card's period-profit result without rerunning the whole app.

        Streamlit fragments keep the user's scroll position stable and avoid the
        visible page flicker that query-string navigation caused in earlier builds.
        """
        row = pd.Series(row_data)
        symbol = str(row.get("Symbol") or "").upper()
        st.markdown(
            '<div class="period-profit-control-title">PROFIT PERIOD • TAP A TIMEFRAME / YEAR</div>',
            unsafe_allow_html=True,
        )
        metric = st.pills(
            "Profit period",
            ...
            key=f"card_profit_period_{symbol}",
            label_visibility="collapsed",
            format_func=lambda item: f"{item}  {format_pct(row.get(item))}",
            help="Uses the Investment Simulator dollar amount above. Selecting a period updates only this card area.",
        )
        st.markdown(
            _period_profit_result_html(row, float(principal), str(metric) if metric else None),
            unsafe_allow_html=True,
        )
