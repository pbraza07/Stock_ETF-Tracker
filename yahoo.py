...
class YahooFinanceProvider(MarketDataProvider):
    ...
    def download_daily_history(self, symbols: Iterable[str]) -> Dict[str, pd.DataFrame]:
        ...
        # auto_adjust=True makes long-horizon comparisons split/dividend adjusted.
        raw = yf.download(
            tickers=symbols,
            period="max",
            interval="1d",
        ...
        # yf.download with multiple tickers returns MultiIndex columns, generally
        # level 0 = OHLC field and level 1 = ticker.
    ...
    def download_live_prices(self, symbols: Iterable[str]) -> Dict[str, float]:
        """Fetch latest intraday observations in one batch where available."""
        symbols = self._clean_symbols(symbols)
        if not symbols:
            return {}
        try:
