...
def main() -> None:
    ...
    universe_meta = {}
    try:
        if UNIVERSE_STATE.exists():
            loaded_universe_meta = json.loads(UNIVERSE_STATE.read_text(encoding="utf-8"))
            if isinstance(loaded_universe_meta, dict):
                universe_meta = loaded_universe_meta
    except Exception as exc:
        print(f"Universe refresh metadata could not be read: {exc}")
    ...
    try:
        UNIVERSE_STATE.unlink(missing_ok=True)
    except Exception:
        pass
    print(
