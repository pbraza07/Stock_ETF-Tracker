    ...
    with portfolio_manage_tab:
        ...
        if save_simulation_clicked and portfolio_save_ready:
            ...
            record = {
                ...
                "pdf_layout": "MarketScope Portfolio Split Simulator v4 - first-page instrument snapshot + combined analytics",
            }
...
with market_tab:
    ...
    with table_view_tab:
        ...
        with table_compare_cols[2]:
            st.caption(
                "Select any number of stock or ETF rows, then add them to Instrument Comparison. Both asset types are supported with no selection limit."
            )
        st.caption(
...
with compare_tab:
    # Unlimited stock + ETF comparison workspace. Selection can be built from Card View,
    ...
    st.markdown(
        ...
        "<small>Compare any number of tracked stocks and ETFs side by side in cards or a sortable table.</small></div>",
        unsafe_allow_html=True,
    )
    if st.session_state.comparison_search_message:
        _cmp_ok, _cmp_msg = st.session_state.comparison_search_message
        (st.success if _cmp_ok else st.warning)(_safe_message(_cmp_msg, "Comparison instrument update completed."))
        st.session_state.comparison_search_message = None
    ...
    # v5.9.20: enhanced comparison search. Local matching covers symbol, name, type,
    # sector, industry and analyst rating. Yahoo lookup can discover an instrument
    # outside the current tracked set and persist it into MarketScope before comparison.
    st.markdown("#### Enhanced Stock & ETF Search")
    search_cols = st.columns([3.2, 1.2, 1.4])
    with search_cols[0]:
        comparison_search_query = st.text_input(
            "Search stocks & ETFs",
            key="comparison_enhanced_search",
            placeholder="Ticker, company/fund name, sector, industry or rating…",
            help="Searches the complete tracked MarketScope universe. Use Yahoo lookup when the instrument is not already tracked.",
        )
    with search_cols[1]:
        comparison_search_type = st.selectbox(
            "Type", ["All", "Stocks", "ETFs"], key="comparison_search_type"
        )
    with search_cols[2]:
        yahoo_lookup_clicked = st.button(
            "🌐 Yahoo lookup",
            key="comparison_yahoo_lookup",
            use_container_width=True,
            disabled=not bool(str(comparison_search_query or "").strip()),
            help="Search Yahoo Finance if the stock or ETF is not already in MarketScope.",
        )

    comparison_search_matches = all_compare_rows.copy()
    if comparison_search_type == "Stocks":
        comparison_search_matches = comparison_search_matches[comparison_search_matches["Type"].astype(str).str.upper().eq("STOCK")]
    elif comparison_search_type == "ETFs":
        comparison_search_matches = comparison_search_matches[comparison_search_matches["Type"].astype(str).str.upper().eq("ETF")]

    _cmp_q = str(comparison_search_query or "").strip().lower()
    if _cmp_q:
        _cmp_mask = pd.Series(False, index=comparison_search_matches.index)
        for _cmp_col in ["Symbol", "Name", "Type", "Sector", "Industry", "Analyst Rating"]:
            if _cmp_col in comparison_search_matches.columns:
                _cmp_mask |= comparison_search_matches[_cmp_col].fillna("").astype(str).str.lower().str.contains(_cmp_q, regex=False, na=False)
        comparison_search_matches = comparison_search_matches.loc[_cmp_mask].copy()
        comparison_search_matches = comparison_search_matches.head(60)

        if not comparison_search_matches.empty:
            search_result_view = comparison_search_matches[[
                c for c in ["Symbol", "Name", "Type", "Sector", "Industry", "Analyst Rating", "Price"]
                if c in comparison_search_matches.columns
            ]].copy()
            search_result_view.insert(0, "Add", False)
            edited_search_results = st.data_editor(
                search_result_view,
                hide_index=True,
                use_container_width=True,
                disabled=[c for c in search_result_view.columns if c != "Add"],
                column_config={
                    "Add": st.column_config.CheckboxColumn("Add", help="Select instruments to add to comparison."),
                    "Price": st.column_config.NumberColumn("Price", format="$%.2f"),
                },
                key="comparison_search_results_editor",
                height=min(380, 76 + len(search_result_view) * 35),
            )
            selected_search_symbols = edited_search_results.loc[edited_search_results["Add"].fillna(False), "Symbol"].astype(str).str.upper().tolist()
            if st.button(
                f"＋ Add selected search results ({len(selected_search_symbols)})",
                key="comparison_add_search_results",
                use_container_width=True,
                disabled=not selected_search_symbols,
            ):
                merged = list(dict.fromkeys(list(st.session_state.compare_symbols) + selected_search_symbols))
                st.session_state.compare_symbols = merged
                st.session_state.stock_compare_selector = merged
                st.session_state.compare_page = 0
                st.rerun()
        else:
            st.info("No tracked MarketScope instrument matched this search. Use Yahoo lookup to search beyond the current universe.")
    else:
        st.caption("Start typing to search the tracked MarketScope stock and ETF universe.")

    if yahoo_lookup_clicked:
        try:
            remote_results = provider.search(str(comparison_search_query).strip(), max_results=12)
        except Exception:
            remote_results = []
        st.session_state.search_results = remote_results
        st.session_state.comparison_search_result_query = _cmp_q

    if st.session_state.search_results and _cmp_q and st.session_state.comparison_search_result_query == _cmp_q:
        st.markdown("**Yahoo Finance lookup results**")
        for _remote in st.session_state.search_results[:12]:
            _rsym = str(_remote.get("symbol") or "").upper().strip()
            _rtype = str(_remote.get("quote_type") or "").upper()
            _rname = str(_remote.get("name") or _rsym)
            _rexchange = str(_remote.get("exchange") or "")
            if not _rsym:
                continue
            remote_cols = st.columns([3.8, 1.2])
            with remote_cols[0]:
                st.markdown(f"**{escape(_rsym)} — {escape(_rname)}**  \n{escape(_rtype)}{(' • ' + escape(_rexchange)) if _rexchange else ''}")
            with remote_cols[1]:
                already_tracked = _rsym in set(all_compare_symbols)
                if already_tracked:
                    if st.button(f"Add {_rsym}", key=f"cmp_add_remote_existing_{_rsym}", use_container_width=True):
                        merged = list(dict.fromkeys(list(st.session_state.compare_symbols) + [_rsym]))
                        st.session_state.compare_symbols = merged
                        st.session_state.stock_compare_selector = merged
                        st.session_state.compare_page = 0
                        st.rerun()
                else:
                    if st.button(f"Track + compare {_rsym}", key=f"cmp_track_remote_{_rsym}", use_container_width=True):
                        with st.spinner(f"Loading {_rsym} history and metadata…"):
                            new_row = cached_single_metrics(_rsym)
                        if not new_row:
                            st.error(f"MarketScope could not load usable market history for {_rsym}.")
                        else:
                            new_row["Universe Source"] = "Yahoo search / manual persistent add"
                            st.session_state.session_rows[_rsym] = new_row
                            if _rsym not in st.session_state.extra_symbols:
                                st.session_state.extra_symbols.append(_rsym)
                            _save_extras_to_url()
                            durable = merge_row_into_snapshot(load_snapshot(), new_row)
                            ok, msg = persist_snapshot(durable, SNAPSHOT_FILE, f"data: add MarketScope comparison instrument {_rsym}", 1)
                            st.session_state.persistence_message = (ok, msg)
                            st.session_state.comparison_search_message = (ok, msg)
                            st.session_state.compare_symbols = list(dict.fromkeys(list(st.session_state.compare_symbols) + [_rsym]))
                            st.session_state.stock_compare_selector = list(st.session_state.compare_symbols)
                            st.session_state.compare_page = 0
                            load_snapshot.clear()
                            load_snapshot_metadata.clear()
                            st.rerun()
    ...
    filtered_compare_symbols = view_filtered["Symbol"].astype(str).str.upper().tolist()
    comparison_controls = st.columns([1.7, 1.6, 1.4, 3.2])
    with comparison_controls[0]:
        if st.button(
            f"＋ Add filtered instruments ({len(filtered_compare_symbols)})",
            key="comparison_add_filtered",
            use_container_width=True,
            disabled=not filtered_compare_symbols,
        ):
            merged = list(dict.fromkeys(list(st.session_state.compare_symbols) + filtered_compare_symbols))
            st.session_state.compare_symbols = merged
            st.session_state.stock_compare_selector = merged
            st.session_state.compare_page = 0
            st.rerun()
    with comparison_controls[1]:
        if st.button(
            "Select all tracked instruments",
            key="comparison_add_all",
            use_container_width=True,
            disabled=not all_compare_symbols,
        ):
            st.session_state.compare_symbols = list(all_compare_symbols)
            st.session_state.stock_compare_selector = list(all_compare_symbols)
            st.session_state.compare_page = 0
            st.rerun()
    ...
    with comparison_controls[3]:
        st.caption(
            f"{len(st.session_state.compare_symbols):,} instrument(s) selected. There is no selection limit; cards paginate 12 at a time while Table Comparison shows the full selected set."
        )
    ...
    compare_selector = st.multiselect(
        ...
        placeholder="Choose stocks / ETFs or use Compare buttons on cards / row selection in Table View",
    )
...
with alerts_tab:
    ...
    - **Unlimited Stock & ETF Comparison:** add stocks or ETFs from the Compare button on any card, select rows in Table View, or use the comparison selector. There is no selection cap. Comparison Cards paginate 12 at a time, while Comparison Table shows the full selected set with all current performance periods, ratings, available stock targets, signals and selected investment-simulation results.
