...
with sector_tab:
    ...
    if sector_stocks.empty:
        ...
        def _render_sector_top_performers_popover(drill_sector: str, key_suffix: str) -> None:
            ...
            drill["_rank"] = pd.to_numeric(drill.get(selected_tf), errors="coerce")
            ...
            selected_returns = pd.to_numeric(drill_table.get(selected_tf), errors="coerce")
            drill_table["Total Profit %"] = selected_returns
            drill_table["Total Profit"] = np.where(
                selected_returns.notna() & np.isfinite(selected_returns),
