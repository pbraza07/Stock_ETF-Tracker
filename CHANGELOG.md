...
- Added recovery, drawdown and exact monthly stress-window evidence where available; clearly labeled annual approximations otherwise.
- Added configurable ranking stability, separate append-only ledgers, daily workflow integration and PDF/Excel reports.
- Added equal/score-weighted 12-stock portfolio handoff to both existing projection strategies, with additive wealth probability and portfolio volatility summaries.
- Added exploratory truncated-history studies and five-year percentile coverage diagnostics. Certified point-in-time ranking validation remains blocked by missing historical universe/sector evidence; no accuracy improvement is claimed.
- Added lazy Future Projection/Favorite Picks rendering, preserved navigation inputs and bounded full-result export caches.
- Repaired the missing NumPy import in snapshot refresh. Packaged code at repository root for Render compatibility without live saved libraries that could overwrite user data.
- See RELEASE_5_11_5.md and VALIDATION_5_11_5.md for installation, measured checks and remaining acceptance gates.
