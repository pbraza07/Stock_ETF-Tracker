    ...
    if sector_stocks.empty:
            ...
            for col, (_, sector_row) in zip(sector_cols, sector_df.iloc[start:start+3].iterrows()):
                with col:
                    ...
                    st.markdown(
                        ...
                        f'<div class="sector-performance-kpis"><div><span>{escape(str(sector_sort_period))}</span><b class="{rank_tone}">{escape(format_pct(rank_val))}</b></div>'
                        f'<div><span>Stocks</span><b>{int(sector_row["Stocks"])}</b><small>tap below</small></div>'
                        f'<div><span>Combined Mkt Cap</span><b>{escape(cap_label)}</b></div></div>'
                    ...
                    if st.button(
                        f'📈 {int(sector_row["Stocks"])} stocks • View top performers',
        ...
        if st.session_state.sector_drilldown:
            ...
            if not drill.empty:
                ...
                drill_logo_urls = cached_instrument_logo_urls(tuple(drill_symbols)) if drill_symbols else {}
                with st.container(border=True, key="sector_top_performers_box"):
                    ...
                    drill_cols = ["Logo", "Symbol", "Name", "Analyst Rating", "Price", sector_sort_period, "YTD", "1Y", "Price Target Average", "Avg Target Implied %", "MarketCap"]
