...
def test_main_table_contract_matches_requested_return_order_and_omits_removed_columns():
    ...
    wanted = ["1D", "1M", "3M", "6M", "YTD", "1Y Avg", "3Y Avg", "5Y Avg", "10Y Avg"]
    positions = [block.index(f'"{name}"') for name in wanted]
    assert positions == sorted(positions)


def test_main_table_uses_content_fit_column_config():
    app = (Path(__file__).resolve().parents[1] / "app.py").read_text(encoding="utf-8")
    main = app[app.index("# Main sortable table"):app.index('st.markdown("### Symbol detail")')]
    assert 'width="stretch"' in main
    assert 'width="small"' not in main
    assert 'width="medium"' not in main
    assert 'Short Buy' in main and 'Long Buy' in main
