...
def _finalize_strategy(
    ...
    summary = {
        ...
        "Probability Positive Total Wealth": float(np.mean(total_wealth_profit > 0.0) * 100.0),
        "Median Total Wealth Return": _median(total_wealth_profit) / float(inputs["starting_investment"]) * 100.0,
...
def run_future_projection(
    ...
    target_weights = np.array([normalized["allocations"][s]/100 for s in model.symbols])
