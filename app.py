...
if "card_sort_choice" not in st.session_state:
    st.session_state.card_sort_choice = _query_scalar_early("sort_choice", "1D")
...
# v5.9.18 migration/defaults: first load of this release starts both market views
# at 1D high-to-low and the Portfolio Split Simulator at $100,000. Users can
# change these controls normally after the initial release default is applied.
if "v5918_defaults_initialized" not in st.session_state:
    if not _query_scalar_early("sort_choice", "").strip():
        st.session_state.card_sort_choice = "1D"
        st.session_state.card_sort_ascending = False
    st.session_state.table_sort_choice = "1D"
    st.session_state.table_sort_direction = "High → Low"
    st.session_state.portfolio_total_amount = 100_000.0
    st.session_state.v5918_defaults_initialized = True
...
def _as_float_or_none(value):
    """Normalize optional numeric values for saved simulation/PDF payloads."""
    try:
        number = float(value)
...
# v5.9.18: make the dynamic Nasdaq >$100B membership refresh visible in the app.
_nasdaq_refresh = _valid_status_text(metadata.get("nasdaq_universe_refreshed_at_display_et"))
_nasdaq_stock_count = int(metadata.get("nasdaq_stock_count") or 0)
_nasdaq_added = list(metadata.get("nasdaq_stocks_added") or [])
_nasdaq_removed = list(metadata.get("nasdaq_stocks_removed") or [])
_nasdaq_added_count = int(metadata.get("nasdaq_stocks_added_count") or len(_nasdaq_added))
_nasdaq_removed_count = int(metadata.get("nasdaq_stocks_removed_count") or len(_nasdaq_removed))
_added_text = ", ".join(str(s) for s in _nasdaq_added[:12]) if _nasdaq_added else "None"
_removed_text = ", ".join(str(s) for s in _nasdaq_removed[:12]) if _nasdaq_removed else "None"
if len(_nasdaq_added) > 12:
    _added_text += f" +{len(_nasdaq_added) - 12} more"
if len(_nasdaq_removed) > 12:
    _removed_text += f" +{len(_nasdaq_removed) - 12} more"
_refresh_display = _nasdaq_refresh or "Pending first successful 6:00 PM ET refresh"
_stock_count_display = f"{_nasdaq_stock_count:,}" if _nasdaq_stock_count else "—"
st.markdown(
    '<div class="universe-refresh-strip">'
    f'<div><span>NASDAQ UNIVERSE LAST REFRESHED</span><b>{escape(_refresh_display)}</b></div>'
    f'<div><span>STOCKS &gt; $100B</span><b>{_stock_count_display}</b></div>'
    f'<div><span>STOCKS ADDED / REMOVED TODAY</span>'
    f'<b><span class="pos-inline">+{_nasdaq_added_count:,}</span> / <span class="neg-inline">-{_nasdaq_removed_count:,}</span></b>'
    f'<small>Added: {escape(_added_text)} • Removed: {escape(_removed_text)}</small></div>'
    '</div>',
    unsafe_allow_html=True,
)
    ...
    if save_simulation_clicked and portfolio_save_ready:
        ...
        for result in portfolio_results:
            ...
            saved_instruments.append({
                ...
                "analyst_rating": (_valid_status_text(meta_row.get("Analyst Rating")) if meta_row is not None else "") or "Not Rated",
                "price_target_low": _as_float_or_none(meta_row.get("Price Target Low")) if meta_row is not None else None,
                "price_target_average": _as_float_or_none(meta_row.get("Price Target Average")) if meta_row is not None else None,
                "price_target_high": _as_float_or_none(meta_row.get("Price Target High")) if meta_row is not None else None,
        ...
        record = {
            ...
            "pdf_layout": "MarketScope Portfolio Split Simulator v4 - combined first page + analyst/target instrument snapshot",
        }
...
with card_view_tab:
    ...
    def _instrument_card_header_html(row: pd.Series, price: str, cap_display: str) -> str:
        """v5.9.7-style card header; the outer native container owns the card border."""
        symbol = escape(str(row.get("Symbol") or ""))
        instrument_type = escape(str(row.get("Type") or ""))
        display_name = escape(_card_display_name(row))
        parts = [
            f'<div id="{_card_anchor_id(str(row.get("Symbol") or ""))}" class="instrument-card-head">',
...
with table_view_tab:
    ...
    with ts1:
        table_sort_choice = st.selectbox(
            "Sort table by",
            table_sort_options,
            index=table_sort_options.index("1D"),
