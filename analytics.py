...
def _price_on_or_before(close: pd.Series, target: pd.Timestamp) -> Optional[float]:
    ...
    if eligible.empty:
        # For newly-listed securities around a target period, do not fabricate a return.
        return None
...
def calculate_performance(history: pd.DataFrame, live_price: Optional[float] = None, as_of: Optional[pd.Timestamp] = None) -> PerformanceResult:
    ...
    def period_return(months: int = 0, years: int = 0) -> Optional[float]:
        ...
        base = _price_on_or_before(close, target)
    ...
    ten_base = _price_on_or_before(close, ten_target)
    five_base = _price_on_or_before(close, five_target)

    # Require near-full horizon coverage before displaying CAGR.
    avg_10y = None
    if inception_date <= ten_target + pd.Timedelta(days=10) and ten_base is not None:
        ten_start_candidates = close.loc[close.index <= ten_target]
        ten_start = pd.Timestamp(ten_start_candidates.index[-1])
        avg_10y = _annualized_return(current, ten_base, ten_start, end)

    avg_5y = None
    if inception_date <= five_target + pd.Timedelta(days=10) and five_base is not None:
        five_start_candidates = close.loc[close.index <= five_target]
        five_start = pd.Timestamp(five_start_candidates.index[-1])
