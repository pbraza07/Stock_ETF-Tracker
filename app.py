...
PERF_COLS = ["1D", "1M", "3M", "6M", "YTD", "1Y Avg", "2Y Avg", "3Y Avg", "4Y Avg", "5Y Avg", "6Y Avg", "7Y Avg", "8Y Avg", "9Y Avg", "10Y Avg"]
...
def _normalize_snapshot(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # v5.4 migration: older snapshots used a point-to-point `1Y` column.
    # Use it as the initial 1Y Avg value until the next historical refresh
    # computes the exact annualized 1-year return.
    if "1Y Avg" not in df.columns and "1Y" in df.columns:
        df["1Y Avg"] = pd.to_numeric(df["1Y"], errors="coerce")
...
def cached_single_metrics(symbol: str) -> dict:
    ...
    return {
        ...
        "2Y Avg": as_percent(perf.avg_2y),
        "3Y Avg": as_percent(perf.avg_3y),
        "4Y Avg": as_percent(perf.avg_4y),
        "5Y Avg": as_percent(perf.avg_5y),
        "6Y Avg": as_percent(perf.avg_6y),
        "7Y Avg": as_percent(perf.avg_7y),
        "8Y Avg": as_percent(perf.avg_8y),
        "9Y Avg": as_percent(perf.avg_9y),
...
def apply_live_overlay(df: pd.DataFrame, prices: Dict[str, float]) -> pd.DataFrame:
    ...
    for idx, row in out.iterrows():
        ...
        if old_is_valid:
            ...
            for col, years in [(f"{y}Y Avg", float(y)) for y in range(1, 11)]:
                value = row.get(col)
                if value is not None and pd.notna(value):
...
def apply_history_refresh(df: pd.DataFrame, histories: Dict[str, pd.DataFrame], symbols: List[str], stamp: str) -> tuple[pd.DataFrame, int]:
    ...
    for symbol in symbols:
        ...
        out.at[idx, "1Y Avg"] = as_percent(perf.avg_1y)
        out.at[idx, "2Y Avg"] = as_percent(perf.avg_2y)
        out.at[idx, "3Y Avg"] = as_percent(perf.avg_3y)
        out.at[idx, "4Y Avg"] = as_percent(perf.avg_4y)
        out.at[idx, "5Y Avg"] = as_percent(perf.avg_5y)
        out.at[idx, "6Y Avg"] = as_percent(perf.avg_6y)
        out.at[idx, "7Y Avg"] = as_percent(perf.avg_7y)
        out.at[idx, "8Y Avg"] = as_percent(perf.avg_8y)
        out.at[idx, "9Y Avg"] = as_percent(perf.avg_9y)
...
<div class="hero">
  <h1>MarketScope</h1>
  <p>Nasdaq stocks > $100B + ETFs • multi-horizon returns • analyst consensus • persistent cloud snapshot</p>
...
with st.sidebar:
    ...
    st.caption("Fast startup uses the last durable snapshot. v5.4 limits the Nasdaq stock universe to market cap strictly above $100B while retaining the requested ETFs and manually persisted symbols. The full scheduled refresh runs every day at 6:00 PM America/New_York.")
...
with rc1:
    refresh_scope = st.segmented_control(
        ...
        help=(
            "The >$100B stock universe is substantially smaller than prior versions. The scheduled 6 PM ET job always refreshes the complete Nasdaq >$100B stock universe plus ETFs."
        ),
    )
with rc2:
...
# Futuristic card navigator — replaces the spreadsheet-style table.
for col in DISPLAY_COLS:
    if col not in filtered.columns:
        filtered[col] = pd.NA

st.markdown("<div class='navigator-title'><span>MARKET NAVIGATOR</span><small>Tap any instrument to open its full performance matrix</small></div>", unsafe_allow_html=True)

sort_choice = st.segmented_control(
    "Sort cards",
    ["Market Cap", "1D", "YTD", "1Y Avg", "5Y Avg", "10Y Avg", "Rating"],
    default="Market Cap",
    key="card_sort",
)
...
if sort_col in filtered.columns:
    if sort_choice == "Rating":
        ...
        filtered = filtered.assign(_sort=filtered[sort_col].map(rank).fillna(0)).sort_values(["_sort", "MarketCap"], ascending=[False, False]).drop(columns="_sort")
    else:
...
for r0 in range(0, len(rows), 4):
    cols = st.columns(4)
    for c, (_, row) in zip(cols, rows[r0:r0+4]):
        ...
        with c:
            st.markdown(
                f'''<div class="instrument-card">
                    <div class="card-top"><span class="ticker">{symbol}</span><span class="asset-type">{row.get("Type", "")}</span></div>
                    <div class="company-name">{name}</div>
                    <div class="price-line">{price}</div>
                    <div class="mini-grid">
                      <div><span>1D</span><b class="{_tone(row.get('1D'))}">{format_pct(row.get('1D'))}</b></div>
                      <div><span>YTD</span><b class="{_tone(row.get('YTD'))}">{format_pct(row.get('YTD'))}</b></div>
                      <div><span>1Y</span><b class="{_tone(row.get('1Y Avg'))}">{format_pct(row.get('1Y Avg'))}</b></div>
                    </div>
                    <div class="card-bottom"><span class="rating-pill {_rating_class(rating)}">{rating}</span><span class="signal-pill">{signal}</span></div>
                </div>''',
                unsafe_allow_html=True,
            )
...
if selected and selected in set(market["Symbol"].astype(str)):
    ...
    st.info("Choose an instrument card to open its full 1D-to-10Y performance view.")

with st.expander("Data methodology, ratings, persistence & schedule"):
    st.markdown(
        """
- **Universe:** Nasdaq Stock Screener rows with market capitalization strictly above **$100 billion**, plus the requested ETF allowlist and any manually persisted symbols.
...
- **Returns:** Yahoo/yfinance adjusted daily market history. 1Y through 10Y Avg are annualized CAGR; YTD/6M/3M/1M/1D are point-to-point adjusted total returns. Stocks do not have NAV. ETF NAV is not shown in the main table; the displayed return columns use adjusted market total-return history.
