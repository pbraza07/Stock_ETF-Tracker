...
with card_view_tab:
    ...
    # v5.9.12: Dedicated Card View stock/ETF search, matching the Table View behavior.
...
def render_card_profit_period_fragment(row_data: dict, principal: float) -> None:
    """Keep the profit-period interaction inside the card area without whole-page flicker."""
    ...
    st.markdown(
        '<div class="embedded-profit-kicker">RETURN TILES • TAP INSIDE THE CARD TO PROJECT PROFIT</div>',
        unsafe_allow_html=True,
    )
    for idx in range(0, len(PERF_COLS), 3):
        row_cols = st.columns(3, gap="small")
        for col, metric in zip(row_cols, PERF_COLS[idx:idx + 3]):
            label = f"{metric} {format_pct(row.get(metric))}"
            active = st.session_state.get(selected_key) == metric
            with col:
                if st.button(
                    label,
                    key=f"card_profit_tile_{symbol}_{metric}",
                    use_container_width=True,
                    type="primary" if active else "secondary",
                ):
                    st.session_state[selected_key] = metric
    chosen_metric = st.session_state.get(selected_key)
    st.markdown(
        _period_profit_result_html(row, float(principal), str(chosen_metric) if chosen_metric else None),
        unsafe_allow_html=True,
    )
...
with table_view_tab:
    ...
    # v5.9.12: Dedicated Table View stock/ETF search. Card View has the same search behavior above.
