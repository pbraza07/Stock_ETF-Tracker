...
def test_profit_tiles_use_pre_rerun_callback_state():
    ...
    assert "args=(symbol, metric)" in APP
    assert 'st.session_state[f"card_profit_period_selected_{clean_symbol}"] = metric' in APP
...
def test_period_controls_and_result_are_inside_keyed_card_shell():
    ...
    selector = APP.index('render_card_profit_period_fragment(row.to_dict(), float(investment_amount))', card_open)
