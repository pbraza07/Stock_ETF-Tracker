...
- Nasdaq >$100B universe refresh time and daily added/removed stock events are now persisted in `data/universe_metadata.json`.
- Portfolio PDF page 1 now includes Symbol, Name, Type, Sector, Analyst Rating, Target Low, Target Average, and Target High for saved instruments.
- Run the scheduled/manual GitHub refresh once after deployment to initialize the universe status metadata.
