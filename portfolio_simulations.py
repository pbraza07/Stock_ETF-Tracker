...
def _target_money(value) -> str:
    numeric = _as_float_or_none(value)
    return f"${numeric:,.2f}" if numeric is not None and numeric > 0 else "-"
...
def build_portfolio_simulation_pdf(record: dict) -> bytes:
    """Build the MarketScope saved portfolio PDF with combined analytics and analyst snapshot on page 1."""
    buf = BytesIO()
    ...
    # ------------------------------------------------------------------
    # Page 1: combined portfolio analytics, combined timeframe returns, and instrument analyst/target snapshot.
    ...
    c.drawString(
        24,
        notes_y,
        "Method: each portfolio return is the saved instrument return weighted by its simulation allocation. A period is shown only when every positive-weight instrument has data.",
    )
    c.drawString(
        24,
        notes_y - 11,
        "10Y CAGR compounds the ten combined calendar-year portfolio returns. Regular yield is allocation-weighted; estimated annual dividend is the sum of saved instrument estimates.",
    )

    # v5.9.18: compact instrument identity / analyst-target table on the first PDF page.
    # Six rows fit legibly alongside the existing combined analytics; larger portfolios
    # continue in the detailed allocation pages rather than shrinking text excessively.
    analyst_title_y = notes_y - 31
    c.setFillColor(accent)
    c.setFont("Helvetica-Bold", 9.2)
    c.drawString(24, analyst_title_y, "PORTFOLIO INSTRUMENTS / ANALYST SNAPSHOT")
    c.setFillColor(muted)
    c.setFont("Helvetica", 6.5)
    c.drawRightString(lwidth - 24, analyst_title_y, "Stock analyst targets are saved from the current MarketScope snapshot; ETF fields may be unavailable")

    analyst_rows = instruments[:6]
    analyst_headers = ["SYMBOL / TYPE", "NAME", "SECTOR", "ANALYST RATING", "TARGET LOW", "TARGET AVG", "TARGET HIGH"]
    analyst_col_w = [72, 190, 120, 95, 86, 86, 86]
    analyst_x = 24
    analyst_header_y = analyst_title_y - 22
    analyst_header_h = 18
    c.setFillColor(HexColor("#0A1A20"))
    c.setStrokeColor(accent)
    c.setLineWidth(0.7)
    c.roundRect(analyst_x, analyst_header_y, sum(analyst_col_w), analyst_header_h, 4, fill=1, stroke=1)
    xpos = analyst_x
    c.setFillColor(accent)
    c.setFont("Helvetica-Bold", 6.1)
    for header, cw in zip(analyst_headers, analyst_col_w):
        c.drawCentredString(xpos + cw / 2, analyst_header_y + 6.3, header)
        xpos += cw

    analyst_row_h = 15
    row_y = analyst_header_y - analyst_row_h
    for idx, item in enumerate(analyst_rows):
        c.setFillColor(card if idx % 2 == 0 else card2)
        c.setStrokeColor(line)
        c.setLineWidth(0.45)
        c.rect(analyst_x, row_y, sum(analyst_col_w), analyst_row_h, fill=1, stroke=1)
        values = [
            _clip_text(f"{item.get('symbol') or '-'} / {item.get('type') or '-'}", 14),
            _clip_text(item.get("name"), 31),
            _clip_text(item.get("sector"), 20),
            _clip_text(item.get("analyst_rating") or "Not Rated", 15),
            _target_money(item.get("price_target_low")),
            _target_money(item.get("price_target_average")),
            _target_money(item.get("price_target_high")),
        ]
        xpos = analyst_x
        for col_idx, (value, cw) in enumerate(zip(values, analyst_col_w)):
            c.setFillColor(text if col_idx < 4 else cyan)
            c.setFont("Helvetica-Bold" if col_idx in {0, 3} else "Helvetica", 5.9)
            c.drawCentredString(xpos + cw / 2, row_y + 5.0, str(value))
            xpos += cw
        row_y -= analyst_row_h

    if len(instruments) > len(analyst_rows):
        c.setFillColor(muted)
        c.setFont("Helvetica-Oblique", 5.8)
        c.drawRightString(lwidth - 24, row_y + 5, f"+ {len(instruments) - len(analyst_rows)} additional instrument(s) continue on following pages")
