...
def build_portfolio_simulation_pdf(record: dict) -> bytes:
    ...
    # Two compact columns allow up to 20 instruments to show symbol/name, sector,
    # analyst rating and low/average/high analyst price targets without shrinking
    ...
    c.setFillColor(muted)
    c.setFont("Helvetica", 7.0)
    c.drawRightString(lwidth - 24, overview_title_y, "Name • sector • analyst rating • low / average / high price targets")
    ...
    for column_no, column_items in enumerate((left, right)):
        ...
        for ridx, item in enumerate(column_items):
            ...
            c.drawRightString(x + overview_w - 6, y - 21, f"LOW {low}   AVG {avg}   HIGH {high}")
            y -= row_h
    ...
    # ------------------------------------------------------------------
    # Pages 3+: individual allocation/result rows, preserving the prior layout.
    # ------------------------------------------------------------------
    width, height = A4
    rows_per_page = 12
