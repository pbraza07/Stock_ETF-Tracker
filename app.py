...
def apply_live_overlay(df: pd.DataFrame, prices: Dict[str, float]) -> pd.DataFrame:
    ...
    for idx, row in out.iterrows():
        ...
        if old_is_valid:
            ...
            for col, years in [("1Y Avg", 1.0), ("3Y Avg", 3.0), ("5Y Avg", 5.0), ("10Y Avg", 10.0)]:
                value = row.get(col)
                if value is not None and pd.notna(value):
...
# Main sortable table — all columns use Streamlit content-fit sizing.
for col in DISPLAY_COLS:
    if col not in filtered.columns:
        filtered[col] = pd.NA

table_df = filtered[DISPLAY_COLS].copy()
for col in ["Short Buy", "Long Buy"]:
    table_df[col] = table_df[col].map(lambda x: "BUY" if bool(x) else "")

styled = (
    table_df.style
    .map(style_return, subset=[c for c in PERF_COLS if c in table_df.columns])
    .map(style_rating, subset=["Analyst Rating"])
    .map(style_signal, subset=["Short Buy", "Long Buy"])
    .set_properties(**{"font-size": "19px", "font-weight": "600", "line-height": "1.35"})
    .set_properties(subset=["Symbol"], **{"font-size": "21px", "font-weight": "800"})
)
st.dataframe(
    styled,
    width="stretch",
    ...
    hide_index=True,
    column_config={
        "Symbol": st.column_config.TextColumn("Symbol", pinned="left"),
        "Name": st.column_config.TextColumn("Name", pinned="left"),
        "Analyst Rating": st.column_config.TextColumn("Analyst Rating"),
        "Short Buy": st.column_config.TextColumn("Short Buy"),
        "Long Buy": st.column_config.TextColumn("Long Buy"),
...
if detail_options:
    ...
    d5.metric("1Y avg", format_pct(detail_row["1Y Avg"]))
    d6.metric("3Y avg", format_pct(detail_row["3Y Avg"]))
    d7.metric("5Y avg", format_pct(detail_row["5Y Avg"]))
    d8.metric("Signal", "LONG BUY" if bool(detail_row.get("Long Buy")) else ("SHORT BUY" if bool(detail_row.get("Short Buy")) else "—"))
...
- **Returns:** Yahoo/yfinance adjusted daily market history. 1Y/3Y/5Y/10Y Avg are annualized CAGR; YTD/6M/3M/1M/1D are point-to-point adjusted total returns. Stocks do not have NAV. ETF NAV is not shown in the main table; the displayed return columns use adjusted market total-return history.
