...
from persistence import format_et, load_remote_metadata, load_remote_snapshot, load_remote_universe_metadata, now_et, persist_snapshot
...
UNIVERSE_BOOTSTRAP_META_FILE = BASE_DIR / "data" / "universe_metadata.bootstrap.json"
...
def load_universe_metadata() -> dict:
    # The GitHub copy is durable and normally freshest; local/bootstrap are fallbacks.
    local = _read_metadata_file(UNIVERSE_META_FILE)
    remote = load_remote_universe_metadata()
    bootstrap = _read_metadata_file(UNIVERSE_BOOTSTRAP_META_FILE)
    for candidate in (remote, local, bootstrap):
...
def _change_symbols_text(values, limit: int = 6) -> str:
    symbols = [str(x).strip().upper() for x in (values or []) if str(x).strip()]
    if not symbols:
        return "None"
    shown = symbols[:limit]
...
universe_updated_display = _valid_status_text(universe_metadata.get("refreshed_at_display_et"))
...
if not universe_updated_display:
    universe_updated_display = "Pending first successful Nasdaq universe refresh"
added_today = universe_metadata.get("stocks_added_today") or []
removed_today = universe_metadata.get("stocks_removed_today") or []
added_count = int(universe_metadata.get("stocks_added_today_count") or len(added_today))
removed_count = int(universe_metadata.get("stocks_removed_today_count") or len(removed_today))
added_text = escape(_change_symbols_text(added_today))
removed_text = escape(_change_symbols_text(removed_today))
st.markdown(
    f"""<div class="update-strip universe-status-strip">
        <span><strong>Nasdaq Universe Last Refreshed:</strong> {escape(universe_updated_display)}</span>
        <span><strong>Stocks Added / Removed Today:</strong> <span class="pos">+{added_count}</span> / <span class="neg">-{removed_count}</span></span>
        <span class="universe-change-detail"><strong>Added:</strong> {added_text} &nbsp; <strong>Removed:</strong> {removed_text}</span>
        <span><strong>Rating update:</strong> {escape(rating_updated_display)}</span>
        <span><strong>Snapshot update:</strong> {escape(snapshot_updated_display)}</span>
    ...
    if save_simulation_clicked and portfolio_save_ready:
        ...
        for result in portfolio_results:
            ...
            saved_instruments.append({
                ...
                "price_target_low": _finite_number(meta_row.get("Price Target Low") if meta_row is not None else None),
                "price_target_average": _finite_number(meta_row.get("Price Target Average") if meta_row is not None else None),
                "price_target_high": _finite_number(meta_row.get("Price Target High") if meta_row is not None else None),
        ...
        record = {
            ...
            "pdf_layout": "MarketScope Portfolio Split Simulator v4 - combined overview + first-page analyst/target instrument snapshot",
        }
