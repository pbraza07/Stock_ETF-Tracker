                ...
                for card_col, (_, row) in zip(card_cols, compare_rows[r0:r0+3]):
                    ...
                    with card_col:
                        ...
                        with st.container(border=True, key=f"comparison_card_{safe_compare_symbol}"):
                            st.markdown(
                                '<div class="comparison-card-logo-row">'
                                + _comparison_logo_html(symbol, comparison_logo_urls.get(symbol, ""))
                                + f'<span><b>{escape(symbol)}</b><small>{escape(str(row.get("Type") or ""))} • {escape(str(row.get("Sector") or "—"))}</small></span></div>',
                                unsafe_allow_html=True,
                            )
                            st.markdown(_instrument_card_html(row, price, cap_display, rating, signal), unsafe_allow_html=True)
                            render_card_profit_period_fragment(row.to_dict(), float(investment_amount))
                            st.markdown(_investment_html(row), unsafe_allow_html=True)
                            st.markdown(_card_bottom_html(rating, signal), unsafe_allow_html=True)
