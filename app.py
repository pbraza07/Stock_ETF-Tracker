...
if "last_profit_scroll_token" not in st.session_state:
    st.session_state.last_profit_scroll_token = ""
...
if "simulation_library_open" not in st.session_state:
    st.session_state.simulation_library_open = False
...
def cached_simulation_pdf(record_json: str) -> bytes:
    return build_portfolio_simulation_pdf(json.loads(record_json))
...
table_search = st.text_input(
    ...
    value=_query_scalar_early("search", ""),
    placeholder="Symbol, name, sector, industry, rating...",
)
...
def _portfolio_horizon_projection(row: pd.Series, principal: float, period_choice: str, include_ytd: bool) -> dict | None:
    ...
    try:
        ...
        return None
    years_requested = max(1, min(10, years_requested))
...
with sim_cols[1]:
    investment_period_choice = st.segmented_control(
        "Investment period",
        ["YTD", *[f"{i}Y" for i in range(1, 11)]],
        default=(_query_scalar_early("sim_period", "10Y") if _query_scalar_early("sim_period", "10Y") in ["YTD", *[f"{i}Y" for i in range(1, 11)]] else "10Y"),
        key="investment_period_choice",
        help="Choose YTD only, or compound 1–10 of the most recent completed calendar years.",
    )
...
with sim_cols[2]:
    include_current_ytd_toggle = st.toggle(
        ...
        help="For 1Y–10Y selections, apply the current YTD return after the completed calendar years. YTD-only always uses YTD by itself.",
    )
    include_current_ytd = True if investment_is_ytd else bool(include_current_ytd_toggle)
with sim_cols[3]:
    st.caption(
        "Investment years: choose YTD for a current-year-only profit calculation, or 1–10 completed calendar years for historical compounding. "
...
# Multi-instrument portfolio split simulator.
...
with port_controls[0]:
    portfolio_period = st.segmented_control(
        ...
        help="YTD uses only the current saved YTD return. 1Y–10Y compounds completed calendar-year returns.",
    )
...
with port_controls[2]:
    portfolio_include_ytd = st.toggle(
        ...
        help="For 1Y–10Y only, optionally apply the current YTD return after the completed-year history.",
    )
...
# v5.9.5 - Save completed portfolio simulations into a durable in-app PDF library.
...
if save_simulation_clicked and portfolio_save_ready:
    ...
    record = {
        ...
        "pdf_layout": "MarketScope Portfolio Split Simulator v3 - combined portfolio first page + instrument analytics",
    }
    # Validate PDF generation before committing the simulation to the durable library.
    try:
        ...
        ok, msg = persist_saved_simulations(
            updated_library,
            BASE_DIR / "data",
            f"data: save MarketScope portfolio simulation {record['id']}",
        )
        st.session_state.simulation_library_message = (ok, msg)
...
if st.session_state.simulation_library_open:
    ...
    st.markdown(
        ...
        "<small>Open this library anytime to download or delete portfolio simulation PDFs.</small></div>",
        unsafe_allow_html=True,
    )
    if not saved_simulations:
        ...
        for rec in saved_simulations:
            ...
            action_cols = st.columns([1.2, 1.0, 4.2])
...
with card_view_tab:
    ...
    st.markdown(
        ...
        "<small>Every card shows short-horizon + 10 calendar-year returns • tap News for catalysts or Open for yearly charts</small></div>",
        unsafe_allow_html=True,
    )
    ...
    def _investment_projection_for_sort(row: pd.Series, principal: float, include_ytd: bool, years_requested: int) -> dict | None:
        ...
        years_requested = max(1, min(10, int(years_requested)))
        if len(newest_to_oldest) < years_requested:
            return None
    ...
    SELECTED_PROFIT_SYMBOL = _query_scalar("profit_symbol").upper()
    SELECTED_PROFIT_PERIOD = _query_scalar("profit_period")
    if SELECTED_PROFIT_PERIOD not in PERF_COLS:
        SELECTED_PROFIT_PERIOD = ""
    ...
    def _period_profit_href(symbol: str, metric: str) -> str:
        params = {
            "profit_symbol": str(symbol).upper(),
            "profit_period": metric,
            "profit_amount": f"{float(investment_amount):.2f}",
            "sim_period": investment_period_choice,
            "include_ytd": "1" if include_current_ytd else "0",
            "card_page": str(st.session_state.card_page),
            "sort_choice": str(st.session_state.card_sort_choice),
            "sort_asc": "1" if st.session_state.card_sort_ascending else "0",
            "instrument": str(instrument_filter or "All"),
            "cap": str(cap_filter or "> $100B"),
        }
        if rating_filter:
            params["ratings"] = ",".join(map(str, rating_filter))
        if signal_filter:
            params["signals"] = ",".join(map(str, signal_filter))
        if selected_sectors:
            params["sectors"] = ",".join(map(str, selected_sectors))
        if table_search:
            params["search"] = str(table_search)
        if st.session_state.get("portfolio_symbols"):
            params["portfolio"] = ",".join(st.session_state.portfolio_symbols)
        try:
            extra = st.query_params.get("extra", "")
            if isinstance(extra, list):
                extra = extra[0] if extra else ""
            if str(extra).strip():
                params["extra"] = str(extra).strip()
        except Exception:
            pass
        return "?" + urlencode(params) + "#" + _card_anchor_id(symbol)
    ...
    def _period_profit_html(row: pd.Series) -> str:
        symbol = str(row.get("Symbol") or "").upper()
        if SELECTED_PROFIT_SYMBOL != symbol or not SELECTED_PROFIT_PERIOD:
            return '<div class="period-profit-hint">Tap any return tile to calculate profit for that exact period.</div>'
        result = _period_profit_projection(row, investment_amount, SELECTED_PROFIT_PERIOD)
        if result is None or result.get("unavailable"):
            return (
                '<div class="period-profit-card"><span>SELECTED PERIOD • ' + escape(SELECTED_PROFIT_PERIOD) + '</span>'
        ...
        return (
            ...
            f'<span>SELECTED PERIOD • {escape(result["period"])} • ${investment_amount:,.2f} invested</span>'
            f'<b>${result["ending_value"]:,.2f}</b>'
            f'<small class="{klass}">Profit {profit:+,.2f} • Return {result["return_pct"]:+.2f}%</small>'
            '</div>'
        )
    ...
    def _instrument_card_html(row: pd.Series, price: str, cap_display: str, rating: str, signal: str) -> str:
        """Build one continuous HTML block so Streamlit never renders nested card markup as code.

        v5.9 inserted an empty price-target fragment for ETFs inside a triple-quoted,
        indented Markdown block.  The resulting blank line could terminate the HTML
        block and make the following indented tags render literally.  Joining compact
        fragments avoids that Markdown edge case for both stocks and ETFs.
        """
        symbol = escape(str(row.get("Symbol") or ""))
        instrument_type = escape(str(row.get("Type") or ""))
        display_name = escape(_card_display_name(row))
        parts = [
            f'<div id="{_card_anchor_id(str(row.get("Symbol") or ""))}" class="instrument-card full-metrics-card">',
            f'<div class="card-top"><span class="ticker">{symbol}</span><span class="asset-type">{instrument_type}</span></div>',
            f'<div class="company-name">{display_name}</div>',
            _stock_sector_html(row),
            ...
            f'<div class="performance-grid">{_performance_cells(row)}</div>',
            _period_profit_html(row),
    ...
    def _performance_cells(row: pd.Series, clickable: bool = True) -> str:
        cells = []
        symbol = str(row.get("Symbol") or "").upper()
        for metric in PERF_COLS:
            value_html = (
            ...
            if clickable:
                active = SELECTED_PROFIT_SYMBOL == symbol and SELECTED_PROFIT_PERIOD == metric
                active_class = " perf-cell-active" if active else ""
                href = escape(_period_profit_href(symbol, metric), quote=True)
                cells.append(
                    f'<a class="perf-cell perf-cell-button{active_class}" href="{href}" title="Calculate profit for {escape(metric)}">'
                    + value_html + '</a>'
                )
            else:
            ...
            with c:
                ...
                with action_cols[2]:
                    if is_etf:
                        ...
                        compare_active = symbol in set(st.session_state.compare_symbols)
                        if st.button(
                            "✓ Comparing" if compare_active else "⚖ Compare",
                            key=f"compare_{symbol}_{page_start}",
                            use_container_width=True,
                            type="primary" if compare_active else "secondary",
                        ):
                            current = list(dict.fromkeys(str(x).upper() for x in st.session_state.compare_symbols))
                            if symbol in current:
                                current = [x for x in current if x != symbol]
                            else:
                                current.append(symbol)
                            st.session_state.compare_symbols = current
                            st.session_state.stock_compare_selector = current
                            st.session_state.compare_page = 0
                            st.rerun()
    ...
    # When a return tile is tapped, return the user to that card after the rerun.
    profit_scroll_token = f"{SELECTED_PROFIT_SYMBOL}|{SELECTED_PROFIT_PERIOD}|{st.session_state.card_page}"
    if SELECTED_PROFIT_SYMBOL and SELECTED_PROFIT_PERIOD and SELECTED_PROFIT_SYMBOL in set(card_rows["Symbol"].astype(str).str.upper()):
        if st.session_state.last_profit_scroll_token != profit_scroll_token:
            st.session_state.last_profit_scroll_token = profit_scroll_token
            target_id = _card_anchor_id(SELECTED_PROFIT_SYMBOL)
            components.html(
                f"""
                <script>
                (() => {{
                  const go = () => {{
                    try {{
                      ...
                      const target = doc.getElementById({json.dumps(target_id)});
    ...
    if selected and selected in set(market["Symbol"].astype(str)):
        ...
        chart_year_options = [str(current_year - offset) for offset in range(0, 11)]
        chart_year = st.segmented_control(
            "Chart year",
            chart_year_options,
            default=str(current_year),
            key=f"chart_year_{selected}",
            help="Choose the current year or any of the prior 10 calendar years. The graph and summary metrics update to that year only.",
        )
...
with table_view_tab:
    ...
    ts1, ts2, ts3 = st.columns([2.2, 1.8, 3.4])
    with ts1:
    ...
    try:
        ...
        selected_table_rows = []
    table_compare_cols = st.columns([1.7, 1.3, 4.0])
    with table_compare_cols[0]:
        if st.button(
            f"⚖ Add selected rows to comparison ({len(selected_table_rows)})",
            key="add_table_rows_to_comparison",
            use_container_width=True,
            disabled=not selected_table_rows,
        ):
            selected_symbols = []
            for row_idx in selected_table_rows:
                if 0 <= int(row_idx) < len(table_df):
                    selected_row = table_df.iloc[int(row_idx)]
                    if str(selected_row.get("Type") or "").strip().upper() == "STOCK":
                        selected_symbols.append(str(selected_row.get("Symbol") or "").upper())
            current = list(dict.fromkeys(str(x).upper() for x in st.session_state.compare_symbols))
            merged = list(dict.fromkeys(current + selected_symbols))
            st.session_state.compare_symbols = merged
            st.session_state.stock_compare_selector = merged
            st.session_state.compare_page = 0
            st.rerun()
    with table_compare_cols[1]:
        if st.button(
            "Clear comparison",
            key="clear_comparison_from_table",
            use_container_width=True,
            disabled=not st.session_state.compare_symbols,
        ):
            st.session_state.compare_symbols = []
            st.session_state.stock_compare_selector = []
            st.session_state.compare_page = 0
            st.rerun()
    with table_compare_cols[2]:
        st.caption(
            "Select any number of table rows, then add them to Stock Comparison. ETF rows are ignored because this comparison section is stock-only."
        )
    st.caption(
...
# Unlimited stock comparison workspace. Selection can be built from Card View,
# Table View row selection, or directly from the selector below. There is no
# artificial selection cap; comparison cards paginate for UI performance.
st.markdown(
    "<div class='comparison-workspace-header'><span>⚖ STOCK COMPARISON</span>"
    "<small>Compare any number of tracked stocks side by side in cards or a sortable table.</small></div>",
    unsafe_allow_html=True,
)
all_stock_rows = market.loc[market["Type"].astype(str).str.upper().eq("STOCK")].copy()
all_stock_rows["Symbol"] = all_stock_rows["Symbol"].astype(str).str.upper().str.strip()
all_stock_rows = all_stock_rows.drop_duplicates("Symbol", keep="last")
all_stock_symbols = sorted(all_stock_rows["Symbol"].tolist())
stock_lookup = all_stock_rows.set_index("Symbol").to_dict(orient="index") if not all_stock_rows.empty else {}

# Drop stale/non-stock symbols while preserving user selection order.
valid_compare_symbols = [
    symbol for symbol in list(dict.fromkeys(str(x).upper() for x in st.session_state.compare_symbols))
    if symbol in set(all_stock_symbols)
]
...
filtered_stock_symbols = view_filtered.loc[
    view_filtered["Type"].astype(str).str.upper().eq("STOCK"), "Symbol"
].astype(str).str.upper().tolist()
comparison_controls = st.columns([1.7, 1.6, 1.4, 3.2])
with comparison_controls[0]:
    if st.button(
        f"＋ Add filtered stocks ({len(filtered_stock_symbols)})",
        key="comparison_add_filtered",
        use_container_width=True,
        disabled=not filtered_stock_symbols,
    ):
        merged = list(dict.fromkeys(list(st.session_state.compare_symbols) + filtered_stock_symbols))
        st.session_state.compare_symbols = merged
        st.session_state.stock_compare_selector = merged
        st.session_state.compare_page = 0
        st.rerun()
with comparison_controls[1]:
    if st.button(
        "Select all tracked stocks",
        key="comparison_add_all",
        use_container_width=True,
        disabled=not all_stock_symbols,
    ):
        st.session_state.compare_symbols = list(all_stock_symbols)
        st.session_state.stock_compare_selector = list(all_stock_symbols)
        st.session_state.compare_page = 0
        st.rerun()
with comparison_controls[2]:
    if st.button(
        ...
        st.session_state.compare_page = 0
        st.rerun()
with comparison_controls[3]:
    st.caption(
        f"{len(st.session_state.compare_symbols):,} stock(s) selected. There is no selection limit; cards paginate 12 at a time while Table Comparison shows the full selected set."
    )

compare_selector = st.multiselect(
    "Stocks to compare (unlimited selection)",
    options=all_stock_symbols,
    key="stock_compare_selector",
    format_func=lambda symbol: (
        f"{symbol} — {stock_lookup.get(symbol, {}).get('Name', symbol)}"
        + (f" • {stock_lookup.get(symbol, {}).get('Sector')}" if str(stock_lookup.get(symbol, {}).get('Sector') or '').strip() not in {'', 'Unknown', 'nan'} else "")
    ),
    placeholder="Choose stocks or use Compare buttons on cards / row selection in Table View",
)
...
comparison_symbols = [s for s in st.session_state.compare_symbols if s in set(all_stock_symbols)]
if not comparison_symbols:
    st.info("Add two or more stocks to build a comparison. You can still compare a single stock while assembling the set.")
else:
    comparison_df = all_stock_rows.loc[all_stock_rows["Symbol"].isin(comparison_symbols)].copy()
    ...
    with compare_cards_tab:
        ...
        with cc_nav[3]:
            st.markdown(f"<div class='page-chip'>{len(comparison_df):,} stocks</div>", unsafe_allow_html=True)
        with cc_nav[4]:
        ...
        for r0 in range(0, len(compare_rows), 3):
            ...
            for card_col, (_, row) in zip(card_cols, compare_rows[r0:r0+3]):
                ...
                comparison_card_html = ''.join([
                    ...
                    f'<div class="card-top"><span class="ticker">{escape(symbol)}</span><span class="asset-type">STOCK</span></div>',
                    f'<div class="company-name">{escape(str(row.get("Name") or symbol))}</div>',
                    _stock_sector_html(row),
                    f'<div class="card-quote-row"><span class="price-line">{escape(price)}</span><span class="cap-line">Mkt Cap {escape(cap_display)}</span></div>',
                    _price_target_html(row),
    ...
    with compare_table_tab:
        ...
        comparison_columns = [
            "Symbol", "Name", "Sector", "Industry", "Price", "Market Cap ($B)",
            "Analyst Rating", "Price Target Low", "Price Target Average", "Price Target High", "Avg Target Implied %",
        ...
        with cs3:
            st.caption(
                "The comparison table includes every selected stock and all current performance periods. Click any column header for another interactive sort."
            )
...
- **Unlimited Stock Comparison:** add stocks from the Compare button on any stock card, select rows in Table View, or use the comparison selector. There is no selection cap. Comparison Cards paginate 12 at a time, while Comparison Table shows the full selected set with all current performance periods, ratings, targets, signals and selected investment-simulation results.
- **Portfolio split simulator:** enter a total amount (default $200,000), select multiple tracked stocks/ETFs, choose equal split or custom percentages, and select YTD or a 1Y–10Y historical horizon. MarketScope calculates each allocation independently, then totals the simulated ending value and profit. Missing history is identified and excluded rather than fabricated. No deposits, withdrawals, taxes, fees, or future returns are assumed.
...
- **Year chart:** below the live chart, opening a card lets you choose the current year or any of the prior 10 calendar years. The plotted adjusted daily closes and the chart summary update to the selected year only.
