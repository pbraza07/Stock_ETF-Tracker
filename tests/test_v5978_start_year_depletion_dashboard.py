...
def test_depletion_dashboard_uses_two_wide_columns_not_six_kpi_cards():
    ...
    assert "Earliest affected start cohort" in section
    assert "Depleted cohorts" in section
    assert "Not depleted" in section
...
def test_pdf_contract_preserved_in_v37():
    marker = (
        "MarketScope Portfolio Split Simulator v37 - v5.9.81 monthly reset + monthly start-year RB/NR depletion dashboard + "
