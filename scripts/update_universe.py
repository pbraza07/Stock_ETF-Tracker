...
def _stock_symbols(df: pd.DataFrame) -> set[str]:
    if df is None or df.empty or "Symbol" not in df.columns:
        return set()
    out = df.copy()
    if "Type" in out.columns:
        out = out[out["Type"].astype(str).str.upper().str.strip().eq("STOCK")]
    return set(out["Symbol"].astype(str).str.upper().str.strip()) - {""}


def _load_universe_metadata() -> dict:
    try:
        if UNIVERSE_META_OUT.exists():
            payload = json.loads(UNIVERSE_META_OUT.read_text(encoding="utf-8"))
...
def _write_universe_metadata(previous: pd.DataFrame, stocks: pd.DataFrame, etf_count: int) -> dict:
    stamp = now_et()
    day = stamp.date().isoformat()
    previous_symbols = _stock_symbols(previous)
    current_symbols = _stock_symbols(stocks)
    added_now = sorted(current_symbols - previous_symbols)
    removed_now = sorted(previous_symbols - current_symbols)

    prior_meta = _load_universe_metadata()
    if str(prior_meta.get("date_et") or "") == day:
        added_today = sorted(set(prior_meta.get("stocks_added_today") or []) | set(added_now))
        removed_today = sorted(set(prior_meta.get("stocks_removed_today") or []) | set(removed_now))
    else:
        added_today = added_now
        removed_today = removed_now

    payload = {
        "refreshed_at_et": stamp.isoformat(),
        "refreshed_at_display_et": format_et(stamp),
        "date_et": day,
        "timezone": "America/New_York",
        "source": "Nasdaq Stock Screener",
        "market_cap_threshold": MIN_MARKET_CAP,
        "stock_count": int(len(current_symbols)),
        "etf_count": int(etf_count),
        "stocks_added_latest_refresh": added_now,
        "stocks_removed_latest_refresh": removed_now,
        "stocks_added_today": added_today,
        "stocks_removed_today": removed_today,
        "stocks_added_today_count": int(len(added_today)),
        "stocks_removed_today_count": int(len(removed_today)),
    }
    UNIVERSE_META_OUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    return payload
...
def main() -> None:
    ...
    tmp.replace(OUT)
    universe_meta = _write_universe_metadata(existing, stocks, len(etfs))
    print(
        f"Universe written: {OUT} ({len(stocks):,} stocks + {len(etfs):,} CSV ETFs = {len(out):,}); "
        f"today +{universe_meta['stocks_added_today_count']}/-{universe_meta['stocks_removed_today_count']} at "
        f"{universe_meta['refreshed_at_display_et']}"
    )
