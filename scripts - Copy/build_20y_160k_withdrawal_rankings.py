...
SOURCE = BASE_DIR / "data" / "annual_performance_20y_160k_source.csv"
OUT_RB = BASE_DIR / "data" / "top250_rebalanced_withdrawal_20y_160k_max10.csv"
OUT_NR = BASE_DIR / "data" / "top250_not_rebalanced_withdrawal_20y_160k_max10.csv"
...
def _year_columns(fieldnames: list[str]) -> list[str]:
    ...
    if len(years) < HORIZON_YEARS:
        raise RuntimeError(
            f"Annual-performance source needs {HORIZON_YEARS} completed calendar years; found {len(years)}."
        )
    return years[:HORIZON_YEARS]
...
def _load_source():
    ...
    if len(stocks) < 4:
        raise RuntimeError("Not enough stocks with complete 20-year history.")
...
def _composite_score(funded: np.ndarray, total_withdrawn: np.ndarray, remaining: np.ndarray) -> np.ndarray:
    # Lexicographic objective encoded numerically:
    # 1) number of full $160K withdrawals funded;
    # 2) total withdrawal cash delivered;
    # 3) remaining portfolio balance.
    # A single additional full withdrawal dominates every possible balance tie-break.
    return funded.astype(np.float64) * 1e15 + total_withdrawn * 1e6 + remaining
...
def _enumerate_scores(stocks, years):
    ...
    sector_groups = {
        sector: np.asarray(indices, dtype=np.uint16)
    ...
    combos = np.empty((total_combos, 4), dtype=np.uint16)
    ...
    for sector_tuple in itertools.combinations(sectors, 4):
        ...
        for start in range(0, total, CHUNK_SIZE):
            ...
            # Rebalanced annually: after each funded withdrawal, the remainder is restored to 25% each.
            balance = np.full(count, START_VALUE, dtype=np.float64)
            funded = np.zeros(count, dtype=np.uint16)
            ...
            for year_index in range(len(years) - 1, -1, -1):
                ...
                active &= balance > 0.0
            rb_scores[cursor : cursor + count] = _composite_score(funded, total_withdrawn, balance)

            # Not rebalanced: holdings retain drift and withdrawals are removed proportionally.
            holdings = np.full((count, 4), START_VALUE / 4.0, dtype=np.float64)
            funded_nr = np.zeros(count, dtype=np.uint16)
            ...
            nr_scores[cursor : cursor + count] = _composite_score(
                funded_nr, total_withdrawn_nr, nr_remaining
            )
...
def _select_top(scores, combos, stocks):
    ...
    if len(selected) != TOP_N:
        raise RuntimeError(
            f"Could only build {len(selected)} combinations under max-{MAX_TICKER_USES} ticker-use rule."
        )
...
def _write_output(selected, uses, stocks, returns, years, rebalance, path):
    ...
    for rank, (_, combo) in enumerate(selected, start=1):
        ...
        for position, index in enumerate(combo, start=1):
            ...
            row[f"Stock {position} Top250 Uses"] = uses[stock["Symbol"]]
        ...
        row.update(
            {
                ...
                "Full 20Y Withdrawal Goal": "Yes" if detail["funded"] == len(years) else "No",
                ...
                "Distinct Tickers in Top 250": distinct,
                "Ranking Window Start": years[-1],
                "Ranking Window End": years[0],
                "Ranking Source": SOURCE.name,
                "Ranking Method": (
                    "Rank by full $160K withdrawals funded, then total cash delivered, then ending balance; "
                    "enforce max 10 Top-250 appearances per ticker."
                ),
            }
        )
    ...
    print(
        f"Wrote {len(output)} rows to {path.name}; {len(uses)} distinct tickers; "
        f"max ticker use {max(uses.values())}; full 20Y goal rows "
        f"{sum(row['Full 20Y Withdrawal Goal'] == 'Yes' for row in output)}."
    )


def main():
    stocks, years = _load_source()
    print(f"Eligible complete-history stocks: {len(stocks)}; window {years[-1]}-{years[0]}")
    returns, combos, rb_scores, nr_scores = _enumerate_scores(stocks, years)
    print(f"Evaluated {len(combos):,} distinct-sector four-stock combinations.")
    rb_selected, rb_uses = _select_top(rb_scores, combos, stocks)
    nr_selected, nr_uses = _select_top(nr_scores, combos, stocks)
