...
def test_market_navigator_groups_requested_controls():
    ...
    # Market Navigator is rendered in two market_tab blocks around the dedicated workspace tabs.
    assert "INVESTMENT SIMULATOR" in APP
    assert "DISPLAY MODE" in APP
    assert "MARKET NAVIGATOR" in APP


def test_pdf_setup_popover_removed_from_app_ui():
    assert 'st.popover("ⓘ PDF Setup"' not in APP
    assert 'os.getenv("MARKETSCOPE_GITHUB_TOKEN")' not in APP or 'PDF Setup' not in APP
