...
with sector_tab:
    ...
    if sector_stocks.empty:
        ...
        def _render_sector_top_performers_popover(drill_sector: str, key_suffix: str) -> None:
            ...
            st.caption("Tap a timeframe header below to re-rank the stocks and recalculate Total Profit and Total Profit %.")
            ...
            # v5.9.34: sector drill-down periods 1Y-20Y are derived cumulative returns,
            ...
            visible_returns = ["1D", "1M", "3M", "6M", "YTD", *[f"{i}Y" for i in range(1, 21)]]
            # Materialize every displayed timeframe using the same calculation path used for
            # ranking/profit so the table never shows blank 1Y-20Y cells simply because those
            # labels are derived rather than physical snapshot columns.
            for timeframe in visible_returns:
                drill_table[timeframe] = _sector_period_return_series(drill_table, timeframe)
            ...
            drill_cols = list(dict.fromkeys([
                ...
                *visible_returns, "Price Target Average", "MarketCap",
            ]))
            ...
            configs.update({metric: st.column_config.NumberColumn(metric, format="%.2f%%") for metric in visible_returns})
            st.dataframe(
