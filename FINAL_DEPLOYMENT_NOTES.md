# MarketScope v5.4.2 deployment notes
...
- Sector buttons now come only from stock-sector values. ETF-only sectors/categories are hidden from the sector filter.
- NAV is removed from the main table.
- Main table uses larger 19px data text, 21px stock/ETF symbols, and 54px rows with mobile readability overrides.
