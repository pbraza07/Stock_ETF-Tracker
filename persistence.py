...
def persist_snapshot(df: pd.DataFrame, local_path: Path, source: str, updated_count: int) -> Tuple[bool, str]:
    ...
    # Manual quote/history refreshes should not erase the last successful Nasdaq
    # universe membership refresh or its added/removed counts.
    prior_metadata = {}
    try:
        prior_path = local_path.parent / "snapshot_metadata.json"
        if prior_path.exists():
            prior_metadata = json.loads(prior_path.read_text(encoding="utf-8"))
        if not isinstance(prior_metadata, dict) or not prior_metadata:
            prior_metadata = load_remote_metadata(timeout=8)
    except Exception:
        prior_metadata = {}
    for key, value in (prior_metadata or {}).items():
        if str(key).startswith("nasdaq_"):
            metadata[key] = value
