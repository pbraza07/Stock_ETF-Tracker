...
def build_portfolio_simulation_pdf(record: dict) -> bytes:
    ...
    # Landscape is intentional so all 15 requested timeframe columns remain legible.
    # ------------------------------------------------------------------
    ...
    table_x = 24
    table_w = lwidth - 48
    cell_w = table_w / len(timeframe_cols)
    header_y = timeframe_title_y - 28
    header_h = 24
    value_h = 34
    ...
    for i, label in enumerate(timeframe_cols):
        x = table_x + i * cell_w
        if i:
