...
def load_snapshot() -> pd.DataFrame:
    # Fast startup normally serves the local/deployed snapshot.  v5.1 also
    # detects the bootstrap file (rows present but zero populated prices) and
    # falls back to the durable GitHub snapshot instead of getting stuck on
    ...
    if _populated_price_count(remote) > _populated_price_count(local):
        return remote
...
def load_snapshot_metadata() -> dict:
    ...
    # A bootstrap metadata file contains null timestamps.  Do not surface
    # those values as the literal word "None"; check GitHub for a real refresh.
    if _valid_status_text(local.get("updated_at_display_et")):
        return local
    remote = load_remote_metadata()
    if isinstance(remote, dict) and _valid_status_text(remote.get("updated_at_display_et")):
        return remote
...
def apply_live_overlay(df: pd.DataFrame, prices: Dict[str, float]) -> pd.DataFrame:
    ...
    for idx, row in out.iterrows():
        ...
        # Critical v5.1 fix: even when the bootstrap snapshot has no prior Price,
        # a successful live quote must still populate the Price column.
...
def apply_history_refresh(df: pd.DataFrame, histories: Dict[str, pd.DataFrame], symbols: List[str], stamp: str) -> tuple[pd.DataFrame, int]:
    """Populate/replace performance metrics from daily adjusted history.

    v5 only overlaid a live quote onto an existing snapshot.  That meant a
    bootstrap snapshot with blank prices could never become populated.  v5.1
    calculates the full return row first, then overlays any intraday quote.
    """
    out = df.copy()
    success = 0
    for symbol in symbols:
        ...
        out.at[idx, "10Y Avg"] = as_percent(perf.avg_10y)
        out.at[idx, "5Y Avg"] = as_percent(perf.avg_5y)
        out.at[idx, "1Y"] = as_percent(perf.perf_1y)
...
with st.sidebar:
    ...
    st.caption("Fast startup uses the last saved snapshot. Refresh now populates blank rows from historical data first. The full scheduled refresh runs every day at 6:00 PM America/New_York.")
...
if st.session_state.last_refresh_summary:
    ...
    if "returned no usable prices" in summary_text.lower():
...
updated_display = _valid_status_text(metadata.get("updated_at_display_et"))
if not updated_display and "Snapshot Updated ET" in market.columns:
    vals = [
        _valid_status_text(x)
        for x in market["Snapshot Updated ET"].dropna().astype(str).tolist()
    ]
    vals = [x for x in vals if x]
    updated_display = vals[-1] if vals else ""
if updated_display:
    st.success(f"Persistent snapshot ready • last refresh: {updated_display} • timezone: U.S. Eastern (America/New_York).")
else:
    st.warning(
        "Initial snapshot has not been populated yet. Click Refresh to build the selected rows now, "
        "or run the 6:00 PM ET GitHub Action for the complete universe."
    )
...
if refresh_now:
    ...
    # Stage 1: historical adjusted data.  This is the critical population step
    # for a new/blank snapshot and supplies 10Y, 5Y, 1Y, YTD, 6M, 3M, 1M and 1D.
    ...
    for start in range(0, total, history_batch_size):
        ...
        try:
            histories = provider.download_daily_history(batch, period="10y")
        except Exception:
            histories = {}
    ...
    # Stage 2: intraday/latest prices.  If historical retrieval is throttled,
    ...
    progress.progress(0.94, text=f"Price data complete • refreshing Nasdaq analyst ratings for {total:,} instruments...")
    ...
    successfully_updated = len(
        set(prices).union(
            symbol for symbol in targets
            if symbol in refreshed.loc[pd.to_numeric(refreshed["Price"], errors="coerce").notna(), "Symbol"].astype(str).tolist()
        )
    )
    refreshed.loc[refreshed["Symbol"].isin(targets) & pd.to_numeric(refreshed["Price"], errors="coerce").notna(), "Snapshot Updated ET"] = stamp
    ok, msg = persist_snapshot(refreshed, SNAPSHOT_FILE, "Manual app refresh", successfully_updated)
    st.session_state.live_prices = {}
    st.session_state.manual_refreshed_at = now_et()
    st.session_state.persistence_message = (ok, _safe_message(msg, "Server snapshot saved; durable GitHub save status unavailable."))

    if successfully_updated:
        st.session_state.last_refresh_summary = (
            f"Manual refresh completed at {format_et(st.session_state.manual_refreshed_at)} • "
            f"{total:,}/{total:,} instruments processed (100%) • {history_success:,} historical rows populated • "
            f"{len(prices):,} latest prices returned • Stocks {target_stocks:,} • ETFs {target_etfs:,}."
        )
    else:
        st.session_state.last_refresh_summary = (
            f"Refresh request completed at {format_et(st.session_state.manual_refreshed_at)}, but the free market-data source returned "
            f"no usable prices for these {total:,} instruments. The last saved snapshot was preserved; retry later or run the scheduled GitHub Action."
        )

    load_snapshot.clear()
    load_snapshot_metadata.clear()
    progress.progress(1.0, text=f"Refresh complete: {total:,} / {total:,} processed (100.0%) • {successfully_updated:,} rows now have price data")
    counter.caption(
        f"Completed at {format_et(st.session_state.manual_refreshed_at)} • historical rows {history_success:,} • "
        f"latest prices {len(prices):,} • rows with price data {successfully_updated:,}"
    )
    st.rerun()
