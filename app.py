...
with card_view_tab:
    ...
    def render_card_profit_period_fragment(row_data: dict, principal: float) -> None:
        """Clickable in-card return tiles with a local, smooth profit update."""
        ...
        st.markdown(
            '<div class="embedded-profit-kicker">CLICK ANY RETURN PERIOD TO CALCULATE PROFIT</div>',
            unsafe_allow_html=True,
        )
    ...
    def _instrument_card_html(row: pd.Series, price: str, cap_display: str, rating: str, signal: str) -> str:
        """Build the static header portion of a market card.

        The native Streamlit bordered container owns the complete card border so
        the clickable return tiles and selected profit result remain inside it.
        """
    ...
    for r0 in range(0, len(rows), 3):
        ...
        for c, (_, row) in zip(cols, rows[r0:r0+3]):
            ...
            with c:
                # One native bordered container contains the complete interactive card:
                # header, every return timeframe, selected profit result, and actions.
